		...
		if(hasCmdArgs){
			...
			if(doPrintHelp){
				...
				if(doBuild){
					...
					String strCode;
					try {
						strCode = ReadWrite.readFile(
								mStrDirWorking+File.separator+mStrFileCode, Charset.defaultCharset());
						laTeXService.buildLaTeXAsync(
								strCode, mStrDirWorking+File.separator+mStrFileOut, mDoEmbedCode);
					} catch (IOException e) {
						Printing.error(
								"Could not read file "
								+mStrDirWorking+File.separator+mStrFileCode+" (IOException)");
					}
				}
				else if(doScan){
